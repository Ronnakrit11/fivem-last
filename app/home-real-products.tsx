...
import { Package, ShoppingCart, X, CheckCircle, AlertCircle, Loader2, Building2, Upload, Shield, Plus, Minus, Trash2 } from "lucide-react";
...
export default function HomeRealProducts({ items }: { items: RealProduct[] }) {
  ...
  const handleConfirmPayment = async () => {
    if (!selectedBankAccount) { setResult({ success: false, message: "กรุณาเลือกวิธีการชำระเงิน" }); return; }
    ...
    if (!buyerBankAccount.trim()) { setResult({ success: false, message: "กรุณากรอกเลขบัญชีผู้โอน" }); return; }
          ...
          <div className="bg-slate-900 border border-white/10 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${
                            selectedBankAccount?.id === bank.id ? "border-emerald-500 bg-emerald-500" : "border-gray-500"
                          }`}>
              ...
              <div className="space-y-4 mb-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1">เลขบัญชีผู้โอน <span className="text-red-500">*</span></label>
